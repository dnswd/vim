{ ... }:
{
  plugins.origami = {
    ...
    luaConfig.post = /* lua */ ''
      ...
      function update_current_fold(row, bufnr)
      ...
      function clear(bufnr)
        fold_ranges[bufnr] = nil
        fold_ranges_map[bufnr] = nil
      end

      function goto_previous_fold()
      ...
      vim.api.nvim_create_autocmd("CursorMoved", {
        callback = function(opts)
          ...
          if row ~= last_row then
            ...
            update_current_fold(row, opts.buf)
          end
        end,
      })
